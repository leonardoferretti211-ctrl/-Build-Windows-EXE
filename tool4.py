# -*- coding: utf-8 -*-
"""
Created on Sat Jan 17 09:01:02 2026
...
# Put the PNG next to tool3.py (recommended) or change this path:
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        # Splitter for better form factor
        splitter = QSplitter(Qt.Orientation.Horizontal)
        splitter.setChildrenCollapsible(False)
        root.addWidget(splitter, 1)

        # LEFT (Scenario) — inside scroll area
        ...
        # RIGHT (Executive results) — inside scroll area
    ...
    def _load_logo(self) -> None:
        ...
        if os.path.exists(p):
            pm = QPixmap(p)
            if not pm.isNull():
                pm = pm.scaledToHeight(48, Qt.TransformationMode.SmoothTransformation)
    ...
    def export_csv(self) -> None:
        if not (self._last_inputs and self._last_times and self._last_phases and self._last_costs and self._last_phase_res and self._last_baseline_h is not None):
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                ...
                w.writerow(["Function", "Phase", "Time/op [min]", "Cost [k€]"])
...
def main() -> None:
    ...
    # Optional: make it look more consistent across Windows machines
