...
def resource_path(relative_path: str) -> str:
    """Works both in dev (python) and in PyInstaller onefile/onedir."""
...
class Inputs:
    ...
    # FTE / cost model (customer-driven)
    crew_today: float
    hse_min_crew: float
    avg_operator_cost_year: float

    # When automated, how many people are still needed in the area (default 1, optional 0)
...
def crew_required_for_phase(
    ...
    - if a function is automated by this phase -> people required becomes automated_people_required (default 1, optional 0)
    - else -> people_today(function)
    - crew required in the area = max(of active functions), and also at least hse_min_crew
    """
    reqs: List[float] = []
    ...
    if not reqs:
        return max(0.0, float(hse_min_crew))

    return max(float(hse_min_crew), max(reqs))
...
def compute_results(
    ...
    # Workload baseline + remaining per phase
    for d in defs:
        if not enabled.get(d.name, True):
            continue
    ...
    for phase_n in (1, 2, 3):
        ...
        # Crew/FTE model
        crew_req = crew_required_for_phase(
            defs=defs,
            enabled=enabled,
            phases=phases,
            people_today=people_today,
            phase_n=phase_n,
            hse_min_crew=inputs.hse_min_crew,
            automated_people_required=inputs.automated_people_required,
        )

        # FTE gained is discrete on the "crew" view; you can keep float if you prefer.
        # Here we keep it as float but compute from discrete crew.
        crew_today = float(inputs.crew_today)
        fte_gained = max(0.0, crew_today - crew_req)
        cost_saved = fte_gained * float(inputs.avg_operator_cost_year)
...
def make_card(title: str) -> Tuple[QGroupBox, QLabel, QLabel, QLabel, QLabel, QLabel, QLabel, QLabel, QLabel]:
    ...
    fte = QLabel("FTE gained / cost saved: -")
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        # Store last computed data for CSV export
        ...
        # Extrapolation assumptions (+ FTE model inputs)
        ...
        # NEW: FTE / cost model toggle + inputs
        self.chk_fte = QCheckBox("Enable FTE & cost model")
        self.chk_fte.setChecked(False)
        self.chk_fte.stateChanged.connect(self.on_toggle_fte)

        self.crew_today = QLineEdit("3")
        self.hse_min_crew = QLineEdit("1")
        self.op_cost_year = QLineEdit("70000")

        # NEW: when automated, people required becomes 1 (default) or 0 (optional)
        self.chk_auto_to_zero = QCheckBox("When automated: allow 0 people in area (otherwise 1)")
        ...
        extra_layout.addWidget(QLabel("Crew in area today"), 3, 0)
        extra_layout.addWidget(self.crew_today, 3, 1)
        extra_layout.addWidget(QLabel("Client / plant dependent"), 3, 2)

        extra_layout.addWidget(QLabel("Minimum crew floor (HSE)"), 4, 0)
        extra_layout.addWidget(self.hse_min_crew, 4, 1)
        extra_layout.addWidget(QLabel("Min crew always present"), 4, 2)

        extra_layout.addWidget(QLabel("Avg operator cost / year [€]"), 5, 0)
        extra_layout.addWidget(self.op_cost_year, 5, 1)
        extra_layout.addWidget(QLabel("Used for annual savings"), 5, 2)

        extra_layout.addWidget(self.chk_auto_to_zero, 6, 0, 1, 3)
        ...
        self.scope_table.setHorizontalHeaderLabels(
            ["Use", "Function", "Automation phase", "Time/op [min]", "Cost [k€]", "People today"]
        )
        ...
        title = QLabel("Executive results – Workload, investment & FTE by phase")
    ...
    def reset_defaults(self) -> None:
        ...
        self.chk_fte.setChecked(False)
        self.crew_today.setText("3")
        self.hse_min_crew.setText("1")
    ...
    def on_toggle_fte(self) -> None:
        ...
        # show/hide the crew/cost inputs
        self.crew_today.setEnabled(show)
        self.hse_min_crew.setEnabled(show)
        self.op_cost_year.setEnabled(show)
        self.chk_auto_to_zero.setEnabled(show)

        # show/hide the per-phase labels
        for card in (self.card_p1, self.card_p2, self.card_p3):
            crew_lbl = card[7]
            fte_lbl = card[8]
            crew_lbl.setVisible(show)
            fte_lbl.setVisible(show)
    ...
    def read_inputs(self) -> Inputs:
        ...
        return Inputs(
            ...
            crew_today=self._f(self.crew_today) if self.chk_fte.isChecked() else 0.0,
            hse_min_crew=self._f(self.hse_min_crew) if self.chk_fte.isChecked() else 0.0,
    ...
    def on_calculate(self) -> None:
        try:
            ...
            if self.chk_fte.isChecked():
                if inputs.crew_today < 0:
                    raise ValueError("Crew in area today must be >= 0.")
                if inputs.hse_min_crew < 0:
                ...
                # automated_people_required is either 1 or 0 (as designed)
    ...
    def _update_phase_card(self, card_tuple, res: PhaseResults, phase_n: int) -> None:
        _, main, remaining, saving, extrap, invest, solutions, crew_lbl, fte_lbl = card_tuple

        show_costs = self.chk_show_costs.isChecked()
        show_fte = self.chk_fte.isChecked()
        ...
        if show_fte:
            crew_lbl.setText(f"Crew required (area):  {res.crew_required:.0f} people")
            fte_lbl.setText(
                f"FTE gained:            {res.fte_gained:.0f}   |   Annual cost reduction: {fmt_eur(res.cost_saved_year)}"
            )
    ...
    def export_csv(self) -> None:
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                ...
                w.writerow(["Enable FTE model", "yes" if show_fte else "no"])

                if show_fte:
                    w.writerow(["Crew in area today", i.crew_today])
                    w.writerow(["HSE min crew", i.hse_min_crew])
                ...
                if show_fte:
                    res_headers.extend(["Crew required [people]", "FTE gained", "Annual cost reduction [€]"])
                ...
                for p in (1, 2, 3):
                    ...
                    if show_fte:
                        row.append(f"{r.crew_required:.0f}")
                        row.append(f"{r.fte_gained:.0f}")
                        row.append(f"{r.cost_saved_year:.2f}")
