...
def resource_path(relative_path: str) -> str:
    """Works in dev and in PyInstaller onefile/onedir."""
...
class Inputs:
    ...
    # Labor model inputs (Extrapolation)
    shifts_per_day: float
    min_crew_per_shift_hse: float
    avg_operator_cost_year: float
...
class OperationDef:
    ...
    # Per-function crew sizing (customer dependent)
    default_crew_manual: float
    default_crew_automated: float
...
def ops_definitions() -> List[OperationDef]:
    ...
    # Defaults you can tune in GUI:
    # manual crew (today): 2
    # automated crew: 1 (or 0 if fully remote)
    return [
        OperationDef("Cylinder manipulation", cylinder, 1, "Phase 1", 100, 2, 1),
        OperationDef("CNT tip cleaning", tip_clean, 3, "Phase 2", 100, 2, 1),
        OperationDef("O₂ lancing", o2_lancing, 4, "Phase 1", 100, 2, 1),
        OperationDef("Plate inspection", plate_inspection, 1, "Phase 2", 100, 2, 1),
        OperationDef("CNT exchange", cnt_exchange, 3, "Phase 1", 100, 2, 1),
        OperationDef("Plate exchange", plate_exchange, 7, "Phase 1", 100, 2, 1),
        OperationDef("Plate cementing", plate_cementing, 2, "Phase 1", 100, 2, 1),
        OperationDef("IN & bottom plate surface cleaning", in_bottom_clean, 3, "Phase 3", 100, 2, 1),
        OperationDef("IN exchange", in_exchange, 15, "Phase 3", 100, 2, 1),
        OperationDef("PP exchange", pp_exchange, 15, "Phase 3", 100, 2, 1),
    ]
...
def baseline_crew_per_shift(enabled: Dict[str, bool], crew_manual: Dict[str, float], hse_floor: float) -> float:
    vals = [crew_manual[k] for k, v in enabled.items() if v and k in crew_manual]
    if not vals:
        return max(0.0, hse_floor)
    return max(hse_floor, max(vals))
...
def crew_required_per_shift_for_phase(
    ...
    for d in defs:
        ...
        is_automated = phase_n >= phase_index(sel_phase)
        req = float(crew_automated.get(d.name, d.default_crew_automated)) if is_automated else float(
            crew_manual.get(d.name, d.default_crew_manual)
        )
        reqs.append(req)
    if not reqs:
        return max(0.0, hse_floor)
    return max(hse_floor, max(reqs))
...
def compute_results(
    ...
    # Labor model (crew per shift driven by function crew sizing)
    base_crew = baseline_crew_per_shift(enabled, crew_manual, inputs.min_crew_per_shift_hse)
    paid_baseline = base_crew * inputs.shifts_per_day
    ...
    for phase_n in (1, 2, 3):
        ...
        crew_req = crew_required_per_shift_for_phase(
            defs=defs,
            enabled=enabled,
            phases=phases,
            crew_manual=crew_manual,
            crew_automated=crew_automated,
            phase_n=phase_n,
            hse_floor=inputs.min_crew_per_shift_hse,
        )
        paid_req = crew_req * inputs.shifts_per_day
        paid_saved = max(0.0, paid_baseline - paid_req)
        cost_saved = paid_saved * inputs.avg_operator_cost_year
        ...
        phase_results[phase_n] = PhaseResults(
            ...
            baseline_crew_per_shift=base_crew,
            crew_per_shift_required=crew_req,
            paid_headcount_baseline=paid_baseline,
            paid_headcount_required=paid_req,
            paid_headcount_saved=paid_saved,
            annual_labor_cost_reduction=cost_saved,
        )
...
def make_card(title: str):
    ...
    labor = QLabel("Labor cut: -")
    labor.setWordWrap(True)

    for w in (remaining, saving, extrap, invest, solutions, labor):
    ...
    return box, main, remaining, saving, extrap, invest, solutions, labor
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        self._last_crew_auto: Dict[str, float] | None = None
        ...
        # Extrapolation + labor model inputs
        extra_box = QGroupBox("Scenario builder – Extrapolation (labor model)")
        ...
        extra_layout.addWidget(QLabel("Number of shifts per day"), 2, 0)
        extra_layout.addWidget(self.shifts_day, 2, 1)
        extra_layout.addWidget(QLabel("e.g. 3 for 24/7"), 2, 2)

        extra_layout.addWidget(QLabel("Minimum crew per shift (HSE floor)"), 3, 0)
        extra_layout.addWidget(self.hse_floor, 3, 1)
        extra_layout.addWidget(QLabel("Minimum staffing constraint"), 3, 2)

        extra_layout.addWidget(QLabel("Average operator cost per year [€]"), 4, 0)
        extra_layout.addWidget(self.op_cost_year, 4, 1)
        extra_layout.addWidget(QLabel("Used for annual labor savings"), 4, 2)
        ...
        self.scope_table.setColumnCount(7)
        ...
        title = QLabel("Executive results – Workload, investment & labor cut by phase")
        ...
        self.lbl_baseline_labor = QLabel("Baseline staffing: -")
        self.lbl_baseline_labor.setStyleSheet("color: #ccc;")
        self.lbl_baseline_labor.setFont(big_font(11, False))

        base_layout.addWidget(self.lbl_baseline_main)
        base_layout.addWidget(self.lbl_baseline_sub)
        base_layout.addWidget(self.lbl_baseline_labor)
        ...
        self.lbl_note = QLabel("Note: Investment is cumulative by phase. Crew per shift is calculated from the scope table and HSE floor.")
        ...
        splitter.setSizes([660, 660])

        # Maps for table widgets
        ...
        self._crew_manual_items: Dict[str, QTableWidgetItem] = {}
        self._crew_auto_items: Dict[str, QTableWidgetItem] = {}
    ...
    def populate_scope_table(self) -> None:
        ...
        for r, d in enumerate(defs):
            ...
            crew_m = QTableWidgetItem(f"{d.default_crew_manual:g}")
            crew_m.setTextAlignment(int(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter))
            self.scope_table.setItem(r, 5, crew_m)
            self._crew_manual_items[d.name] = crew_m

            crew_a = QTableWidgetItem(f"{d.default_crew_automated:g}")
            crew_a.setTextAlignment(int(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter))
            self.scope_table.setItem(r, 6, crew_a)
            self._crew_auto_items[d.name] = crew_a
    ...
    def reset_defaults(self) -> None:
        ...
        for d in defs:
            ...
            self._crew_manual_items[d.name].setText(f"{d.default_crew_manual:g}")
            self._crew_auto_items[d.name].setText(f"{d.default_crew_automated:g}")
    ...
    def read_inputs(self) -> Inputs:
        return Inputs(
            ...
            shifts_per_day=self._f(self.shifts_day),
            min_crew_per_shift_hse=self._f(self.hse_floor),
            avg_operator_cost_year=self._f(self.op_cost_year),
        )

    def read_scope(self) -> Tuple[Dict[str, bool], Dict[str, float], Dict[str, str], Dict[str, float], Dict[str, float], Dict[str, float]]:
        enabled: Dict[str, bool] = {}
        times: Dict[str, float] = {}
        phases: Dict[str, str] = {}
        costs: Dict[str, float] = {}
        crew_manual: Dict[str, float] = {}
        crew_auto: Dict[str, float] = {}
        ...
        for name, item in self._crew_manual_items.items():
            crew_manual[name] = float(item.text().strip())

        for name, item in self._crew_auto_items.items():
            crew_auto[name] = float(item.text().strip())

        return enabled, times, phases, costs, crew_manual, crew_auto
    ...
    def on_calculate(self) -> None:
        try:
            ...
            enabled, times, phases, costs, crew_manual, crew_auto = self.read_scope()
            ...
            if inputs.min_crew_per_shift_hse < 0:
                raise ValueError("Minimum crew per shift (HSE floor) must be >= 0.")
            ...
            for k, v in crew_auto.items():
                if enabled.get(k, True) and v < 0:
                    raise ValueError(f"Crew per shift (automated) must be >= 0 (check '{k}').")
            ...
            baseline_h, phase_res = compute_results(inputs, times, phases, costs, enabled, crew_manual, crew_auto)
        ...
        self._last_crew_auto = crew_auto
        self._last_baseline_h = baseline_h
        self._last_phase_res = phase_res

        # Update baseline
        self.lbl_baseline_main.setText(fmt_h_day(baseline_h))
        base_crew = phase_res[1].baseline_crew_per_shift  # same across phases
        paid_base = phase_res[1].paid_headcount_baseline
        self.lbl_baseline_labor.setText(
            f"Baseline staffing: crew/shift = {base_crew:.0f}  |  total paid headcount = {paid_base:.0f}"
        )
    ...
    def _update_phase_card(self, card_tuple, res: PhaseResults, phase_n: int) -> None:
        _, main, remaining, saving, extrap, invest, solutions, labor = card_tuple
        ...
        if self.chk_show_costs.isChecked():
            if phase_n == 1:
        ...
        labor.setText(
            f"Labor cut:            crew/shift = {res.crew_per_shift_required:.0f}  |  paid headcount saved = {res.paid_headcount_saved:.0f}"
            f"  |  annual cost reduction = {fmt_eur(res.annual_labor_cost_reduction)}"
        )
    ...
    def export_csv(self) -> None:
        if not (
            ...
            and self._last_enabled and self._last_crew_manual and self._last_crew_auto
        ):
        ...
        crew_auto = self._last_crew_auto
        ...
        try:
            with open(path, "w", newline="", encoding="utf-8") as f:
                ...
                headers += ["Crew/shift (manual)", "Crew/shift (automated)"]
                w.writerow(headers)
                ...
                for d in ops_definitions():
                    ...
                    row.append(f"{crew_manual.get(name, d.default_crew_manual):.2f}")
                    row.append(f"{crew_auto.get(name, d.default_crew_automated):.2f}")
                    w.writerow(row)
                ...
                res_headers = [
                    ...
                    "Crew/shift baseline",
                    "Crew/shift required",
                    "Paid headcount baseline",
                    "Paid headcount required",
                    "Paid headcount saved",
                ...
                for p in (1, 2, 3):
                    ...
                    row.extend([
                        f"{r.baseline_crew_per_shift:.0f}",
